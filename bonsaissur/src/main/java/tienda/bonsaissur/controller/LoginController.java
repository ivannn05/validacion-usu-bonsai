...
util.ficheroLog("Ocurrio un error en cerrarSesion :"+e.getMessage());
	...
	public ResponseEntity<Void> validarUsu(@RequestParam String correo, @RequestParam String token,
			HttpSession session) {
		try {
			util.ficheroLog("Usuario entro en valida usuario");
			String respuesta = service.validartokenUsu(correo, token);
